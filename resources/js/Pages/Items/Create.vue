...
// この form オブジェクトは、フォームの入力フィールド（v-model ディレクティブを使用してバインドされたフィールド）と双方向のデータバインディングを行うために使用されます。つまり、ユーザーがフォームのフィールドに入力を行うと、
// form オブジェクトの対応するプロパティ（name、memo、price）に入力された値が反映されます。同様に、form オブジェクトの各プロパティの値が変更されると、入力フィールドの値も更新されます。
const form = reactive({
    name: null,
    memo: null,
    price:null
