...
// RFM分析
        ...
        // $eachCount 配列と $rank 変数を初期化します。
        $eachCount = []; // Vue側に渡すようの空の配列
        $rank = 5; // 初期値5
        
        for($i = 0; $i < 5; $i++)
        {
          // forループを使って、各ランクごとのデータを $eachCount 配列に追加しています。
          // ループ内の処理では、連想配列を作成し、'rank' => $rank のように 'rank' キーには
          // 現在のランクの値が入ります（初期値は5で、1回のループごとに1ずつ減少します）。
          // 'r'、'f'、'm' キーにはそれぞれ $rCount[$i]、$fCount[$i]、$mCount[$i] の値が入ります。
          // これらはRFM分析の結果であり、各ランクに属する会員の数が格納されています。
